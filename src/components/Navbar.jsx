    ...
    <nav className="navbar navbar-expand-lg sticky-top" style={{
      ...
      <div className="container">
        ...
        <Link className="navbar-brand d-flex align-items-center gap-2" to="/" style={{ textDecoration: 'none' }}>
          <div style={{
            width: 36, height: 36, background: '#0a2342', borderRadius: 8,
            display: 'flex', alignItems: 'center', justifyContent: 'center'
          }}>
            <i className="bi bi-droplet-fill text-white" style={{ fontSize: 16 }}></i>
          </div>
          <div>
            <span style={{ fontFamily: "'Playfair Display', serif", fontWeight: 700, fontSize: 18, color: '#0a2342' }}>
              Jumbohydro
            </span>
            <span style={{ fontSize: 11, color: '#6c757d', display: 'block', lineHeight: 1, marginTop: -2 }}>
              INDIA
            </span>
          </div>
        </Link>
        ...
        <div className="collapse navbar-collapse" id="navMenu">
          <ul className="navbar-nav mx-auto gap-1">
            {[['/', 'Home'], ['/products', 'Products'], ['/about', 'About'], ['/contact', 'Contact']].map(([path, label]) => (
              <li className="nav-item" key={path}>
                <Link className="nav-link px-3" to={path} style={{ color: '#2d2d2d', fontWeight: 500, fontSize: 15 }}>
                  {label}
                </Link>
              </li>
            ))}
          ...
          <div className="d-flex align-items-center gap-2">
            ...
            <select
              ...
              style={{ width: 'auto', fontSize: 13, border: '1px solid #dee2e6', borderRadius: 8 }}
            >
              {CURRENCIES.map(c => (
            ...
            {/* Cart */}
            <Link to="/cart" className="btn btn-outline-secondary btn-sm position-relative" style={{ borderRadius: 8 }}>
            ...
            {user ? (
              <div className="dropdown">
                <button className="btn btn-sm dropdown-toggle d-flex align-items-center gap-2"
                  ...
                  style={{ background: '#0a2342', color: '#fff', borderRadius: 8, border: 'none' }}>
                  <i className="bi bi-person-circle"></i>
                  {user.name.split(' ')[0]}
                </button>
                <ul className="dropdown-menu dropdown-menu-end shadow-sm border-0" style={{ borderRadius: 12 }}>
