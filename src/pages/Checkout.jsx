...
export default function Checkout() {
  ...
  const [form, setForm] = useState({
    ...
    street: '', landmark: '', city: '', state: '', pin: '', country: 'India'
  });
  ...
  const handleOrder = async (e) => {
    ...
    try {
      ...
      if (window.Cashfree) {
        const cashfree = await window.Cashfree({ mode: import.meta.env.VITE_CASHFREE_ENV === 'PROD' ? 'production' : 'sandbox' });
  ...
  const fields = [
    ...
    { name: 'email', label: 'Email', placeholder: 'john@example.com', col: 12, type: 'email' },
    { name: 'street', label: 'Street Address', placeholder: '123 Marine Drive', col: 12 },
    { name: 'landmark', label: 'Landmark (optional)', placeholder: 'Near post office', col: 12 },
        ...
        <div className="row g-4">
          <div className="col-lg-7">
            <div className="p-4" style={{ background: '#fff', borderRadius: 20, border: '1px solid #e8e8e8' }}>
              <h5 style={{ fontWeight: 700, color: '#0a2342', marginBottom: 24 }}>Shipping Address</h5>
                  ...
                  {fields.map(f => (
                    <div key={f.name} className={`col-md-${f.col}`}>
                      ...
                      <input type={f.type || 'text'} name={f.name} value={form[f.name]}
                        onChange={handleChange} placeholder={f.placeholder} required={f.name !== 'landmark'}
                        className="form-control" style={{ borderRadius: 10, fontSize: 14 }} />
                    </div>
                  ))}
                </div>
                <button type="submit" disabled={loading} className="btn w-100 mt-4"
                  style={{ background: '#0a2342', color: '#fff', borderRadius: 12, padding: '14px', fontWeight: 600, fontSize: 16 }}>
                  {loading ? 'Processing...' : `Pay ${getCurrencySymbol(currency)}${total.toFixed(2)} via Cashfree`}
                </button>
              </form>
            </div>
          </div>
          ...
          <div className="col-lg-5">
            <div className="p-4" style={{ background: '#f8f9fa', borderRadius: 20 }}>
              <h5 style={{ fontWeight: 700, color: '#0a2342', marginBottom: 20 }}>Order Summary</h5>
              {cartItems.map(item => {
                const price = item.product?.price?.[currency] || 0;
                return (
                  <div key={item.product?._id} className="d-flex gap-3 mb-3 align-items-center">
              ...
              <div className="mt-4 p-3" style={{ background: '#e8f0f8', borderRadius: 12 }}>
                <p style={{ fontSize: 13, color: '#0a2342', margin: 0 }}>
                  <i className="bi bi-shield-check me-2"></i>
                  Secured by <strong>Cashfree Payments</strong>. Your payment info is encrypted.
                </p>
              </div>
            </div>
          </div>
