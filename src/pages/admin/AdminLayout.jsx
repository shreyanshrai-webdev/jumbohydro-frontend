...
export default function AdminLayout() {
  ...
  return (
    <div className="d-flex" style={{ minHeight: '100vh', fontFamily: "'DM Sans', sans-serif" }}>
      {/* Sidebar */}
      <div style={{
        width: 240, background: '#0a2342', color: '#fff',
        ...
        <div className="p-4" style={{ borderBottom: '1px solid rgba(255,255,255,0.1)' }}>
          <div className="d-flex align-items-center gap-2">
            <div style={{
              ...
              <i className="bi bi-droplet-fill" style={{ color: '#0a2342', fontSize: 16 }}></i>
            </div>
            <div>
              <div style={{ fontFamily: "'Playfair Display', serif", fontWeight: 700, fontSize: 15, color: '#fff' }}>
                Jumbohydro
              </div>
              <div style={{ fontSize: 10, color: '#7eb8e8', letterSpacing: 1 }}>ADMIN PANEL</div>
            </div>
          </div>
        </div>
          ...
          {NAV_ITEMS.map(({ to, icon, label }) => {
            ...
            return (
              <Link key={to} to={to} style={{ textDecoration: 'none' }}>
                <div className="d-flex align-items-center gap-3 px-3 py-2 mb-1" style={{
                  borderRadius: 10,
        ...
        <div className="p-3" style={{ borderTop: '1px solid rgba(255,255,255,0.1)' }}>
          <div className="d-flex align-items-center gap-2 mb-3 px-2">
            <div style={{
              ...
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              fontWeight: 700, fontSize: 13, color: '#0a2342'
            }}>
              {user?.name?.charAt(0).toUpperCase()}
            </div>
            <div>
              <div style={{ fontSize: 13, fontWeight: 600, color: '#fff' }}>{user?.name}</div>
              <div style={{ fontSize: 11, color: '#7eb8e8' }}>Administrator</div>
            </div>
          </div>
          <Link to="/" style={{ textDecoration: 'none' }}>
            <div className="d-flex align-items-center gap-2 px-3 py-2" style={{
              color: '#a8c4dc', fontSize: 13, borderRadius: 10,
